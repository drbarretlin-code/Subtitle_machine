...
若手機與電腦不在同一個網路環境（例如：手機使用 5G，電腦在公司），您可以使用「內網穿透」工具：
1.  **安裝 Localtunnel**: 在電腦執行 `npx localtunnel --port 5173`。
2.  **取得網址**: 程式會配發一個如 `https://fluffy-cat-99.loca.lt` 的公開網址。
3.  **手機瀏覽**: 在手機開啟該網址即可跨網段連線。
4.  **注意**: 此模式下，後端 API 仍需在電腦端保持執行。
